...
Il dispatching dei thread che identificano i giocatori non e' modificata a livello applicazione, pertanto demanda tale compiti allo scheduler del sistema operativo sottostante. Le versioni recenti di Windows e OSX adottano lo scheduler \textit{Multilevel feedback queue}, invece le versioni di Linux successive alla 3.0 (compresa) utilizzano \textit{Completely Fair Scheduler} (CFS). Entrambi gli scheduler utilizzano un approccio basato su quanti di tempo anche se con strutture completamente diverse:

\begin{itemize}
	\item CFS si serve di un albero rosso-nero inserendo i nuovi a destra, quindi i thread che hanno appena eseguito, e prelevandoli da sinistra, quelli con meno utilizzo della cpu;
	\item Multilevel feddback queue utilizza code FIFO multiple, precisamente 100, ed i thread sono accodati in coda e prelevati in testa.\\
\begin{itemize}

Quello che emerge e' che i thread eseguono per una quantita' di tempo prestabilita e poi vengono riaccodati e, nel caso baso, rieseguono una volta che tutti gli altri thread in attesa hanno avuto accesso alla CPU. Alcuni casi particolari vengono applicati in presenza di job molto brevi, utilizzo di I/O, etc.\\

In \ref{sec:analisi_concorrenza_velocita} e' discussa un routing di esecuzione frutto del modello e delle scelte architetturali, esso non rappresenta la reale esecuzione che e' invece soggetta alle scelte di dispatching da parte dello scheduler del sistema operativo. La simulazione della velocita' avviene tramite delay assolute con tempi di attesa proporzionali alla durata di un turno, ma non e' detto che ogni turno sia eseguito in modo contiguo in quanto la durata di una singola esecuzione e' superiore della durata del quanto di tempo assegnatogli. Di conseguenza lo schema rappresentato in Figura~\ref{fig:hyperperiod_normal} risulta maggiormente frammentato. Nonostante questo l'utilizzo di delay assolute e le modalita' di calcolo dell'iperperiodo assicurano ad ogni giocatore (e relativo thread) tempo sufficiente per eseguire i propri turni.\\

In \ref{sec:ai_problematiche} viene fatto riferimento all'utilizzo di un motore Prolog esterno rispetto all'applicazione. A livello implementativo questo avviene tramite la libreria \textit{Ada Util} che permette di aprire un terminale dall'interno del software tramite il comando \textit{pipe} ed avviare un Buffer per leggere l'output del motore fino a che non termina. Questo comporta che il thread non si sospende in attesa della risposta dell'IA, di conseguenza non e' soggetto ad ulteriori prerilasci oltre a quelli previsti dallo scheduler. Inoltre la stima del tempo necessario per l'esecuzione di un turno incorpora il caso pessimo della risoluzione delle clausole da parte di Prolog, di conseguenza il comportamento finora descritto non ne risente.
